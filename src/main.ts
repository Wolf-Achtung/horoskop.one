import { attachAutocomplete } from './geocode';
import { fetchReading } from './readingApi';
import { renderShareCardPNG } from './sharecard';
import { drawZodiacWheel } from './zodiac';
const byId=(id:string)=>document.getElementById(id) as HTMLInputElement;
function isoToGerman(input:string){if(!/^\d{4}-\d{2}-\d{2}$/.test(input))return input;const[y,m,d]=input.split('-');return`${d}.${m}.${y}`}
function approxMap(v:string){const map:Record<string,string>={morning:'morgens',noon:'mittags',evening:'abends',night:'nachts','':'unbekannt'};return map[v]??'unbekannt'}
function setStatus(m:string){byId('status').textContent=m}function setDebug(o:any){const b=byId('debug');const p=byId('debugPre');b.hidden=false;p.textContent=JSON.stringify(o,null,2)}
function stableSeed(s:string){let h=2166136261>>>0;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619)}return h>>>0}
function readForm(){const dRaw=(byId('birthDate').value||'').trim();const p=(byId('birthPlace').value||'').trim();const lat=(byId('birthLat').value||'').trim();const lon=(byId('birthLon').value||'').trim();const t=(byId('birthTime').value||'').trim();const a=(byId('timeApprox').value||'').trim();const tf=(byId('timeFrame').value||'week').trim();const mode=(byId('mode').value||'balanced').trim();const readingType=(byId('readingType')?.value||'classic').trim();if(!dRaw||!p)throw new Error('Bitte Datum und Ort angeben.');const birthDate=/^\d{4}-\d{2}-\d{2}$/.test(dRaw)?isoToGerman(dRaw):dRaw;const period=tf==='today'?'day':tf;const payload:any={birthDate,birthPlace:p,period,approxDaypart:approxMap(a),mode,readingType};if(t&&/^\d{1,2}:\d{2}$/.test(t))payload.birthTime=t;if(lat&&lon)payload.coords={lat:Number(lat),lon:Number(lon)};const seed=stableSeed((dRaw||'')+'|'+(p||''));payload.seed=seed;payload.tone=mode;
  // Merge Kosmischer Mixer state if mounted
  const mx=(window as any).__mixerState;
  if(mx&&typeof mx==='object'){
    if(mx.weights) payload.mixer=mx.weights;
    if(mx.mode) payload.tone=mx.mode;
  }
  return payload}
function permalinkFromForm(){const d=byId('birthDate').value;const p=byId('birthPlace').value;const t=byId('birthTime').value;const a=byId('timeApprox').value;const tf=byId('timeFrame').value;const lat=byId('birthLat').value;const lon=byId('birthLon').value;const mode=byId('mode').value;const rt=byId('readingType')?.value||'classic';const seed=stableSeed((d||'')+'|'+(p||''));const u=new URL(location.href);u.searchParams.set('d',d);u.searchParams.set('p',p);if(t)u.searchParams.set('t',t);if(a)u.searchParams.set('a',a);if(tf)u.searchParams.set('tf',tf);if(lat&&lon){u.searchParams.set('lat',lat);u.searchParams.set('lon',lon)}u.searchParams.set('mode',mode);u.searchParams.set('rt',rt);u.searchParams.set('seed',String(seed));byId('permalink').href=u.toString()}
async function onSubmit(e){e.preventDefault();const btn=byId('btnAsk');try{btn.disabled=true;btn.textContent='Lädt …';setStatus('Berechnung läuft – bitte warten (kann bis zu 60 s dauern) …');const payload=readForm();const data=await fetchReading(payload);if(data?.error){setStatus('Antwort mit Fehler erhalten');setDebug(data)}else{setStatus('Fertig')}renderReading(data);byId('shareCardPreview').hidden=false;await drawShareCardFromReading(data);permalinkFromForm()}catch(err:any){const msg=err?.name==='AbortError'?'Zeitüberschreitung – bitte erneut versuchen.':(err?.message||String(err));setStatus('Fehler: '+msg);setDebug({error:msg,stack:err?.stack})}finally{btn.disabled=false;btn.textContent='Sterne fragen'}}
function renderReading(data:any){const out=byId('readingOut');out.innerHTML='';
  // Show reading type label if available
  if(data?.meta?.readingLabel){const h=document.createElement('div');h.className='reading-type-badge';h.textContent=data.meta.readingLabel;out.appendChild(h)}
  const make=(t:string,x:string,chips?:string[])=>{const b=document.createElement('div');b.className='reading-block';let html=`<strong>${t}</strong><div>${x}</div>`;if(chips?.length){html+='<div class="reading-chips">'+chips.map(c=>`<span class="reading-chip">${c}</span>`).join('')+'</div>'}b.innerHTML=html;out.appendChild(b)};
  if(data?.summary)make('Zusammenfassung',data.summary);
  if(data?.highlights?.length)make('Highlights',data.highlights.slice(0,5).map((s:string)=>'• '+s).join('<br>'));
  if(data?.sections?.length)for(const s of data.sections)make(s.title||'Abschnitt',s.text||'',s.chips);
  if(data?.disclaimer){const d=document.createElement('div');d.className='reading-disclaimer';d.textContent=data.disclaimer;out.appendChild(d)}
  if(!out.children.length)make('Antwort',typeof data==='string'?data:JSON.stringify(data))}
async function drawShareCardFromReading(data){const canvas=byId('shareCanvas');const d=byId('birthDate').value;const p=byId('birthPlace').value;const tf=byId('timeFrame').value;const title='Dein Moment unter den Sternen';const bullets=(data?.highlights?.length?data.highlights.slice(0,3):(data?.sections||[]).slice(0,3).map(s=>s.title||'Impulse'));const png=await renderShareCardPNG(canvas,{title,subtitle:`${p} · ${d} · ${tf}`,bullets});const a=byId('btnDownload');a.href=png;a.hidden=false}
function attachEvents(){document.getElementById('astro-form').addEventListener('submit',onSubmit);document.getElementById('btnShare').addEventListener('click',async()=>{byId('shareCardPreview').hidden=false;await drawShareCardFromReading({highlights:['Atem holen. Fokus finden.','Kleiner mutiger Schritt.','Heute freundlich zu dir sein.']})})}
function hydrateFromURL(){const url=new URL(location.href);const map:Record<string,string>={d:'birthDate',p:'birthPlace',t:'birthTime',a:'timeApprox',tf:'timeFrame',mode:'mode',rt:'readingType',lat:'birthLat',lon:'birthLon'};for(const[q,id]of Object.entries(map)){const v=url.searchParams.get(q);const el=document.getElementById(id) as HTMLInputElement;if(v&&el)el.value=v}permalinkFromForm()}
function init(){try{drawZodiacWheel(document.getElementById('zodiac-canvas'))}catch(e){console.warn('zodiac init failed',e)}try{attachAutocomplete({input:document.getElementById('birthPlace'),menu:document.getElementById('place-suggest'),onPick(place){document.getElementById('birthPlace').value=place.label;(document.getElementById('birthLat') as HTMLInputElement).value=String(place.lat??'');(document.getElementById('birthLon') as HTMLInputElement).value=String(place.lon??'');permalinkFromForm()}})}catch(e){console.warn('autocomplete init failed',e)}attachEvents();hydrateFromURL()}
document.addEventListener('DOMContentLoaded',init);
