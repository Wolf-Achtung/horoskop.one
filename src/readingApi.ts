export async function fetchReading(payload){
  ...
  if(!res.ok){
    const res2=await fetch('https://horoskopone-production.up.railway.app/readings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
